...
const AlertWarning = ({
  ...
  return (
    <>
      <div
        className={`flex w-full rounded-[10px] border-l-6 border-[#FFB800] bg-[#FEF5DE] dark:bg-[#1B1B24] dark:bg-opacity-30 ${size == "NORMAL" ? "px-7 py-8 md:p-9" : size == "SMALL" ? "px-3 py-2" : ""} ${className}`}
      >
        {icon ? (
        ...
        <div className="w-full">
          <h5
            className={`${size == "NORMAL" ? "mb-3.5" : size == "SMALL" ? "mb-1" : ""} text-lg font-bold leading-[22px] text-[#9D5425]`}
          >
            {title ?? "Alert title"}
          </h5>
