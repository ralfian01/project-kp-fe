...
const privilegesRules: Array<string> = ["SCHOLARSHIP_GENERAL_VIEW"];
...
export default async function Page() {
  ...
  const { data: applicationGeneral } = await getApplicant("general");

  return (
    <Dashboard>
      {hasPrivileges ? (
        <>
          <Breadcrumb pageName="Permohonan - Umum" />
                ...
                <Table
                  ...
                  head={
                    <TableRow>
                      ...
                      {applicationGeneral && applicationGeneral.length >= 1 ? (
                  ...
                  body={
                    applicationGeneral && applicationGeneral.length >= 1 ? (
                      applicationGeneral.map((item: any, key: number) => (
                        <TableRow
                          key={key}
                        ...
                        >
                          ...
                          <TableCell className="text-center">
                            <div
                              className={`inline-block w-full whitespace-nowrap rounded-full ${["DRAFT", "PENDING"].indexOf(item.status) >= 0 ? "bg-gray-200 text-gray-600" : ["APPROVED"].indexOf(item.status) >= 0 ? "bg-blue-400 text-white" : ["FINALIZED"].indexOf(item.status) >= 0 ? "bg-green-500 text-white" : ["REVISE"].indexOf(item.status) >= 0 ? "bg-orange-500 text-white" : "bg-red text-white"} px-3 py-1 text-center text-sm`}
                            >
                          ...
                          <TableCell className="sticky right-0 z-10  bg-gray-200">
                            <div className="flex">
                              {["DRAFT", "REVISE"].indexOf(item.status) >= 0 ? (
                                <>
                                  <ProposeButton applicantData={item} />

                                  <ButtonDefault
                                    label=""
                                    link={`/application/general/${item.applicant_id}`}
