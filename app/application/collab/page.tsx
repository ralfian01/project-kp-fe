...
const privilegesRules: Array<string> = ["SCHOLARSHIP_COLLAB_VIEW"];
...
export default async function Page() {
  ...
  const { data: applicationCollab } = await getApplicant("collab");

  return (
    <Dashboard>
      {hasPrivileges ? (
        <>
          <Breadcrumb pageName="Permohonan - Kerjasama" />
                ...
                <Table
                  ...
                  head={
                    <TableRow>
                      ...
                      {applicationCollab && applicationCollab.length >= 1 ? (
                  ...
                  body={
                    applicationCollab && applicationCollab.length >= 1 ? (
                      applicationCollab.map((item: any, key: number) => (
                        <TableRow
                          key={key}
                            ...
                            <div className="flex">
                              {["DRAFT", "REJECT"].indexOf(item.status) >= 0 ? (
                                <>
                                  ...
                                  <ButtonDefault
                                    label=""
                                    link={`/application/collab/${item.applicant_id}`}
